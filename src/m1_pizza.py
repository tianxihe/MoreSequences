...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  100 red dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  200 slices, thin (thickness=1) black lines on ' \
            'yellow pizza.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  4 segments with thick black lines on lime ' \
            'green circle'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(250, 200), 180)
    circle.outline_thickness = 3
    ...
    for k in range(number_of_segments):
        if k!=number_of_segments-1:
         lines=rg.Line(points[k],points[k+1])
          ...
          lines=rg.Line(points[number_of_segments-1],points[0])
        lines.thickness=thickness
        lines.color=color
        lines.attach_to(window)
    window.render()
